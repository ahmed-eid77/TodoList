<div wire:key='{{ $todo->id }}' class="todo mb-5 card px-5 py-6 bg-white col-span-1 border-t-2 border-blue-500 hover:shadow">
    <div class="flex justify-between space-x-2">
        ...
        <div class="flex items-center space-x-2">
            <button class="text-sm text-teal-500 font-semibold rounded hover:text-teal-800" wire:click='edit({{ $todo->id }})'>
            ...
            <button class="text-sm text-red-500 font-semibold rounded hover:text-teal-800 mr-1" wire:click.prevent='delete({{ $todo->id }})'>
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                    stroke="currentColor" class="w-4 h-4">
                    <path stroke-linecap="round" stroke-linejoin="round"
